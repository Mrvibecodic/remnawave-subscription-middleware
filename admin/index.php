...
'userflags':{t:'Колонки «Статус» и «Конфиг»',h:'<h4>Статус</h4><p>Статус пользователя из панели. Тег <code>ГРЕЙС</code> — юзер активен и прямо сейчас состоит в грейс-скваде (сверяется по API панели с UUID сквада из вкладки «Грейс-сквад»). Если грейс закончился и юзер стал EXPIRED — показывается EXPIRED.</p><h4>Конфиг</h4><p>Результат: что реально отдаётся в приложение. <b>Прослойка</b> — подписка заблокирована оверрайдом, вместо тела origin отдаются ремарки блокировки. <b>Панель</b> — реальный конфиг origin (в том числе для истёкших: их обрабатывает грейс-сквад панели, тело прослойка не подменяет). <b>Панель + Грейс</b> — юзер в грейсе: конфиг тоже реальный, от origin, но с нодами грейс-сквада.</p>'}
};
