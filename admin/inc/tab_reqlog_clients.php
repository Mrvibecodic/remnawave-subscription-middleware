    ...
    <div class="card">
        ...
        <p class="muted">Прослойка узнаёт версию клиента из <code>User-Agent</code> и сравнивает с актуальной из источника. В логе запросов у отставших версий появляется точка, а в карточке — строка «Версия». Проверка идёт лениво и фоном: после загрузки лога запросов уходит один ajax, который опрашивает не больше двух источников и не чаще раза в сутки на строку — сама страница из-за этого не ждёт сеть. Подписку это не трогает — запросы уходят только со страниц админки.</p>
        ...
        <form method="post">
            ...
            <p class="muted" style="margin:.2rem 0 .9rem;font-size:.8rem">
                <b>Ключ UA</b> — первое слово из <code>User-Agent</code> в нижнем регистре. <b>Адрес</b>: для GitHub и Codeberg — <code>owner/repo</code>, для App Store — числовой ID приложения, для Google Play и F-Droid — имя пакета вида <code>com.example.app</code>; произвольные ссылки не принимаются.
