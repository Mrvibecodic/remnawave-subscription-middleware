    ...
    @media(max-width:900px){
        ...
        .rl-tbl tr.rowb td:nth-child(5){order:-1;padding-bottom:.4rem}
        .rl-tbl tr.rowb td:nth-child(5)::before{content:none}
        ...
        .rl-tbl tr.rowb td:nth-child(5) .u-cell{align-items:flex-start;text-align:left}
    ...
    <div class="card">
        ...
        <form method="get" class="rl-flt">
            ...
            <?php if ($rl_f['q'] !== '' || $rl_f['dec'] !== '' || $rl_f['hours'] !== 24): ?>
            <a class="btn ghost" href="?tab=reqlog" data-tip="Сбросить фильтры">Сброс</a>
            <?php endif; ?>
        </form>
        <div class="rl-wrap">
            <table class="rl-tbl">
                <colgroup><col class="c-tgl"><col class="c-time"><col class="c-dec"><col class="c-as"><col class="c-user"><col class="c-client"><col class="c-cnt"></colgroup>
                <thead><tr><th></th><th>Время</th><th>Решение</th><th>Доп. подписка</th><th>Пользователь</th><th>Клиент</th><th>Запросов / сутки</th></tr></thead>
                <tbody id="rlBody" class="lp-cap"><?= reqlog_render_rows($reqlog, $rl_ctx) ?></tbody>
            </table>
        </div>
        <div id="rl_pgrBot" class="pgr-bot"></div>
        <p class="muted" style="margin-bottom:0">Строка раскрывается по клику. «Устройство» берётся из того, что клиент прислал в User-Agent. Имя пользователя подтягивается из панели по shortUuid.</p>
    </div>
