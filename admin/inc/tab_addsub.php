    ...
    <section class="<?= coll_cls('addsub_intro') ?>" data-coll="addsub_intro">
        ...
        <div class="coll-body">
            ...
            <p class="muted">Подмешивание идёт <b>только пока основная подписка активна</b>: истекла/заблокирована — узлы второй пропадают сами (страховка от криво выставленного времени). При исчерпании <b>трафика</b> второй подписки вместо её узлов подмешивается заглушка-метка. В <b>грейсе</b> подмешивание тоже выключено: истёкший видит только грейс-сквад, а вторая подписка своего грейса не получает и после истечения просто выпадает из выдачи.</p>
            <p class="muted"><b>Авто</b> — имя второго пользователя = <b>полное</b> имя первого плюс суффикс, совпадение точное и только в той же панели: <code>tg_12345_a1b2c</code> → <code>tg_12345_a1b2c<?= h(addsub_suffix()) ?></code>. Рандомный хвост имени, если панель или бот его генерит, копируется как есть. Промах поиска кэшируется, поэтому после создания или переименования второго пользователя авто подхватит его не сразу — до срока «кэша дискавери» ниже. <b>Ручное</b> — кнопкой «+» во вкладке «Пользователи»: вводится адрес второй подписки, имя и панель любые. Настройки ниже общие для обоих режимов.</p>
        </div>
    </section>
    ...
    <div class="card">
        <form method="post" data-autosave>
            ...
            <div class="row" style="margin-top:1rem">
                <div><label>Суффикс имени для авто <span class="hint">имя B = полное имя A + суффикс; пустым нельзя</span></label><input type="text" name="addsub_username_suffix" value="<?= h(addsub_suffix()) ?>" placeholder="_addsub"></div>
        ...
        <div class="coll-body">
            <div class="subwrap">
                <div class="subedit">
                    ...
                    <p class="muted" style="margin:.2rem 0">Создай в панели второго пользователя: имя = <b>полное</b> имя основного + суффикс <code><?= h(addsub_suffix()) ?></code>, вместе с хвостом имени — например <code>tg_12345_a1b2c</code> → <code>tg_12345_a1b2c<?= h(addsub_suffix()) ?></code>. Сравнение точное, с учётом регистра; статусы <code>DISABLED</code> и <code>EXPIRED</code> авто пропускает. Пользователь — <b>без</b> <code>telegramId</code>, со своим лимитом трафика и сквадом. Для ручного режима имя любое — адрес его подписки вводится кнопкой «+».</p>
